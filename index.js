...
commander
  ...
  .option(
    "-d, --dir <path>",
    'Directory to install and "watch" (i.e. set up watchers for) [dir]'
  )
  .option(
    "-i, --useIgnoreFile",
    "Don't create watchers for for top level files in the root .npmignore/.gitignore file of the local dir"
  )
...
try {
  ...
  errorLog(
    'directory "' + pathToRepo + '" does not contain a "package.json" file'
  );
...
function isNodeModulesOrHidden(path) {
  return (
    path.indexOf(".") === 0 ||
    path.substr(-1 * "node_modules".length) === "node_modules"
  );
}
...
function mkDirPromise(newDirPath) {
  if (dirCreationPromises[newDirPath]) {
    debug(
      "Found promise to create directory, for path " +
        newDirPath +
        " in progress"
    );
    ...
    fs.access(parentDir, fs.F_OK, function (err) {
      if (err) {
        ...
        if (err.code === "ENOENT") {
          ...
          debug(
            "Encountered unexpected error creating directory:" + err.message
          );
          reject(err);
        }
      } else {
...
function promiseCopyFile(readFilePath, writeFilePath) {
  assert.ok(
    path.isAbsolute(readFilePath),
    "readFilePath must be absolute path"
  );
  assert.ok(
    path.isAbsolute(writeFilePath),
    "writeFilePath must be absolute path"
  );
  return new Promise(function (resolve, reject) {
    debug("reading file to copy:", readFilePath);
    return promiseReadFile(readFilePath)
      .then(function (contents) {
        return createParentDirsIfNotExist(writeFilePath).then(function () {
    ...
    function (err) {
      ...
      npm.commands.pack([pathToPkg], function (err) {
        ...
        if (!localModulePkg.name || !localModulePkg.version) {
          return callback(
            new Error(
              "Local npm project repo must specify version and name in package.json"
            )
          );
        }
        var tarFileName =
          localModulePkg.name + "-" + localModulePkg.version + ".tgz";
        tarFileName = tarFileName.replace(/^@/, "").replace(/\//, "-");
        var tarballPath = "./" + tarFileName;
        debug("installing packed local repo directory: " + tarFileName);
...
function execNpmInstall(pathToPkg, callback) {
  ...
  npmLocalPackInstall(pathToPkg, function (err) {
    if (err) {
      ...
      log(
        'installed node_module "%s" successfully, creating watchers for top-level files/folders',
        modulePackage.name
      );
      callback();
    }
...
function removeDeletedDir(srcDir) {
  ...
  removeDir(dirToRemove, function (err) {
    if (err) {
      errorLog(
        "Error removing directory that was removed in source repo",
        dirToRemove
      );
      throw err;
    } else {
      log(
        "directory deleted in repo, deleted matching directory in package:",
        srcDir
      );
    }
  });
  ...
  var statFn = function (path) {
    ...
    fs.access(path, fs.R_OK, function (err) {
      ...
      fs.lstat(path, function (err, stat) {
        ...
        debug(
          "found top level file to exclude from symlink replacement:",
          path
        );
        if (onFoundTopFile) {
...
function onNpmInstall() {
  if (commander.useIgnoreFile) {
    debug(
      "ignore file will be used to find additional top-level " +
    ...
    debug(
      "ignore file will not be used for finding additional " +
